...
class ScannerApp:
    BG = '#1e1e2e'; BG2 = '#181825'; BLUE = '#89b4fa'; RED = '#f38ba8'; GRN = '#a6e3a1'; FG = '#cdd6f4'
    ...
    def _build_ui(self):
        ...
        tk.Label(top_frame, text="Arduino (Détecteur):", bg=self.BG2, fg=self.BLUE, font=('', 10, 'bold')).grid(row=0, column=3, padx=10)
        ...
        self.btn_start_scan = tk.Button(buttons_frame, text="▶ Lancer Scan", bg=self.GRN, font=('', 10, 'bold'), command=self.start_scan)
        ...
        self.ax.set_xlabel('Tension Balayage (Volts)', color='white')
    ...
    def _refresh_ports(self):
        ...
        if pts: self.cbPort.set(pts[0])

    def connect_arduino(self):
        if self.ser is None or not self.ser.is_open:
            try:
                self.ser = serial.Serial(self.cbPort.get(), 115200, timeout=1.0)
                time.sleep(2)
    ...
    # --- LOGIQUE DE SCAN ---
    def start_scan(self):
        if not self.galvos.connected or self.ser is None or not self.ser.is_open:
            messagebox.showwarning("Attention", "Connectez le Red Pitaya ET l'Arduino.")
            return
    ...
    def process_serial_and_loop(self):
        ...
        # 2. Préparation du point suivant
        ...
        if self.current_scan_voltage <= self.voltage_max:
            ...
            try:
                # Récupère la valeur, remplace la virgule par un point au cas où, et convertit en ms
                total_time_ms = int(float(self.spin_delay.get().replace(',', '.')) * 1000)
            except ValueError:
                total_time_ms = 500 # Valeur de sécurité si l'entrée est invalide

            # On soustrait le temps déjà écoulé (50ms mouvement + 20ms lecture série)
            delay_ms = total_time_ms - 70
            if delay_ms < 10: 
                delay_ms = 10 # Sécurité pour ne pas bloquer l'interface si l'utilisateur met un temps trop court

            self.root.after(delay_ms, self.scan_step)
        else:
            self.stop_scan()
            messagebox.showinfo("Fini", "Le balayage est terminé !")
